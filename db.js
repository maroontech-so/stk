...
function migrateLegacyJson() {
  ...
  const insertUser = db.prepare(`
    INSERT OR IGNORE INTO users
      (id, phone, name, email, country, receive_mode, till, paybill, account,
       require_pin, use_biometrics, dark_theme, balance, created_at, updated_at)
    VALUES (@id,@phone,@name,@email,@country,@receiveMode,@till,@paybill,@account,
  ...
  const migrate = db.transaction(() => {
    for (const user of Object.values(legacy?.users || {})) {
      insertUser.run({
        id: user.id, phone: user.phone || user.id, name: user.name || null, email: user.email || null,
        country: user.country || null, receiveMode: user.receiveMode || null, till: user.till || null,
        paybill: user.paybill || null, account: user.account || null, requirePin: user.requirePin ? 1 : 0,
        useBiometrics: user.useBiometrics ? 1 : 0, darkTheme: user.darkTheme ? 1 : 0,
        balance: Number(user.balance) || 0, createdAt: user.createdAt || now, updatedAt: user.updatedAt || now
      });
    }
    for (const tx of legacy?.transactions || []) {
      insertTx.run({
        trackingId: tx.trackingId, userId: tx.userId, phone: tx.phone, amount: Number(tx.amount) || 0,
        reference: tx.reference || null, description: tx.description || null, status: tx.status || 'PENDING',
        checkoutRequestId: tx.checkoutRequestId || null, palplussResponse: JSON.stringify(tx.palplussResponse || null),
        callbackData: JSON.stringify(tx.callbackData || null), receiptNumber: tx.receiptNumber || null,
        resultDesc: tx.resultDesc || null, balanceApplied: tx.balanceApplied ? 1 : 0,
        createdAt: tx.createdAt || now, updatedAt: tx.updatedAt || now
      });
    }
...
function parseTransaction(row) {
  if (!row) return row;
  return {
    trackingId: row.tracking_id, userId: row.user_id, phone: row.phone, amount: row.amount,
    ...
    callbackData: JSON.parse(row.callback_data || 'null'), receiptNumber: row.receipt_number,
    resultDesc: row.result_desc, balanceApplied: Boolean(row.balance_applied),
